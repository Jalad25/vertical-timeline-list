...
abstract class SettingInput implements SettingDisplay {
  ...
  constructor(inputType: SettingInputType, disabled: boolean, hidden: boolean, name?: string, description?: string) {
    this.name = name;
...
class ColorInput extends SettingInput {
  ...
  constructor(disabled: boolean, hidden: boolean, colorSpace: ColorSpace, value?: string, onChange?: (plugin: VerticalTimelineListPlugin, value: string) => void, name?: string, description?: string) {
    super(SettingInputType.color, disabled, hidden, name, description);
...
class BooleanInput extends SettingInput {
  ...
  constructor(disabled: boolean, hidden: boolean, value?: boolean, onChange?: (plugin: VerticalTimelineListPlugin, value: boolean) => void, name?: string, description?: string) {
    super(SettingInputType.boolean, disabled, hidden, name, description);
...
class ThemesCSSColorSetting implements SettingDisplay {
  ...
  constructor(plugin: VerticalTimelineListPlugin, key: string, index: number, disabled: boolean, hidden: boolean, name?: string, description?: string) {
    this.name = name ?? "";
    this.description = description ?? "";
              ...
              (timelinePiece: string) => {
                ...
                configurations.forEach(
                  (configuration, index: number) => {
                    (this[property][timelinePiece] as ThemesCSSColorSetting[]).push(new ThemesCSSColorSetting(plugin, timelinePiece, index, false, false, configuration.name, configuration.description));
                  }
                )
              }
            )
          ...
          case "timelineCSSToggles": //Set CSS Settings for Toggles
            Object.keys(plugin.configuration[property]).forEach(
              (timelinePiece: string) => {
                ...
                configurations.forEach(
                  (configuration, index: number) => {
                    (this[property][timelinePiece] as ToggleCSSSetting[]).push(new ToggleCSSSetting(plugin, timelinePiece, index, false, false, configuration.name, configuration.description));
                  }
                )
              }
            )
...
export class VerticalTimelineListPluginSettingsTab extends PluginSettingTab {
  ...
  private addSettingSection(containerEl: HTMLElement, sectionHeader: string, sectionDescription?: string) : HTMLElement {
    ...
    const header = this.addSettingDisplay(container, sectionHeader, sectionDescription);
  ...
  private addThemeColorTable(containerEl: HTMLElement, rows: ThemesCSSColorSetting[], name?: string, description?: string) {
    ...
    rows.forEach((row: ThemesCSSColorSetting) => {
      ...
      this.addSettingDisplay(td, row.name, row.description);
      td = tr.createEl("td");
  ...
  private addInput(containerEl: HTMLElement, input: SettingInput) : Setting {
    const inputSetting = this.addSettingDisplay(containerEl, input.name, input.description);
  ...
  private addSettingDisplay(containerEl: HTMLElement, name?: string, description?: string) : Setting {
